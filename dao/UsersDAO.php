...
class UsersDAO extends DAO
{
    ...
    public function update($id, $email, $mother, $partner, $duedate, $type, $group_id) {
        $sql = "UPDATE `kb_users` SET `email` = :email, `mother` = :mother, `partner` = :partner, `duedate` = :duedate, `type` = :type, `group_id` = :group_id WHERE `id` = :id";
        $stmt = $this->pdo->prepare($sql);
        $stmt->bindValue(':id', $id);
        $stmt->bindValue(':email', $email);
        $stmt->bindValue(':mother', $mother);
    ...
    public function updatePhoto($id, $photo_url) {
        $sql = "UPDATE `kb_users` SET `photo_url` = :photo_url WHERE `id` = :id";
        $stmt = $this->pdo->prepare($sql);
        $stmt->bindValue(':id', $id);
        $stmt->bindValue(':photo_url', $photo_url);
        if ($stmt->execute()) {
